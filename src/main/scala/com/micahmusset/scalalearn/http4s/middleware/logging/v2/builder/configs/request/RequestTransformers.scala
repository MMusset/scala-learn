...
object RequestTransformers {
  ...
  def message[F[_]: Applicative]: Kleisli[F, Request[F], String] =
    Kleisli[F, Request[F], String](request => s"Request ${request.method} ${request.uri.renderString}".pure[F])

  def static[F[_]: Applicative](
      name: String,
      value: String
  ): Kleisli[F, Request[F], Map[String, String]] =
    Kleisli[F, Request[F], Map[String, String]](_ => Map(name -> value).pure[F])

  def method[F[_]: Applicative](name: String): Kleisli[F, Request[F], Map[String, String]] =
    Kleisli[F, Request[F], Map[String, String]](request => Map(name -> request.method.name).pure[F])
  ...
  def uri[F[_]: Applicative](
    ...
    Kleisli[F, Request[F], Map[String, String]] { request =>
      val filterAndMask =
